...
//		M3508AngleIntegral(&M3508_CHASSIS_MOTOR_REAL_INFO[3]);
		...
		case M3508_CHASSIS_MOTOR_ID_5://D轮子
		{ 
		...
		case M3508_CHASSIS_MOTOR_ID_7://3508夹取机构
		{ 
...
//			M3508AngleIntegral(&M3508_CHASSIS_MOTOR_REAL_INFO[5]);	
		...
		{ 
			M2006_CHASSIS_MOTOR_REAL_INFO[0].ANGLE= (msg -> Data[0] << 8) | msg -> Data[1];  // 转子机械角度
			M2006_CHASSIS_MOTOR_REAL_INFO[0].RPM     = (msg -> Data[2] << 8) | msg -> Data[3];  // 实际转子转速
			M2006_CHASSIS_MOTOR_REAL_INFO[0].CURRENT = (msg -> Data[4] << 8) | msg -> Data[5];  // 实际转矩电流
			M2006AngleIntegral_special_left_turn(&M2006_CHASSIS_MOTOR_REAL_INFO[0]);			//??????????????为啥这里用的是普通初始化？？？？？
		}; break;

		case M2006_CHASSIS_MOTOR_ID_1://2006电机（左边）
		{ 
			M2006_CHASSIS_MOTOR_REAL_INFO[1].ANGLE= (msg -> Data[0] << 8) | msg -> Data[1];  // 转子机械角度
			M2006_CHASSIS_MOTOR_REAL_INFO[1].RPM     = (msg -> Data[2] << 8) | msg -> Data[3];  // 实际转子转速
			M2006_CHASSIS_MOTOR_REAL_INFO[1].CURRENT = (msg -> Data[4] << 8) | msg -> Data[5];  // 实际转矩电流
...
void m3508_update_m3508_info_can2(CanRxMsg *msg)
{
	...
	switch(msg -> StdId)  // 检测标准ID
	{
		case CAN2_M2006_MOTOR_ID_1://2006电机一
		{ 
			M2006_CHASSIS_MOTOR_REAL_INFO[2].ANGLE= (msg -> Data[0] << 8) | msg -> Data[1];  // 转子机械角度
			M2006_CHASSIS_MOTOR_REAL_INFO[2].RPM     = (msg -> Data[2] << 8) | msg -> Data[3];  // 实际转子转速
			M2006_CHASSIS_MOTOR_REAL_INFO[2].CURRENT = (msg -> Data[4] << 8) | msg -> Data[5];  // 实际转矩电流
			M2006AngleIntegral_special_left_push(&M2006_CHASSIS_MOTOR_REAL_INFO[2]);			
		}; break;

		case CAN2_M2006_MOTOR_ID_2://2006电机二
		{ 
			M2006_CHASSIS_MOTOR_REAL_INFO[3].ANGLE= (msg -> Data[0] << 8) | msg -> Data[1];  // 转子机械角度
			M2006_CHASSIS_MOTOR_REAL_INFO[3].RPM     = (msg -> Data[2] << 8) | msg -> Data[3];  // 实际转子转速
			M2006_CHASSIS_MOTOR_REAL_INFO[3].CURRENT = (msg -> Data[4] << 8) | msg -> Data[5];  // 实际转矩电流
...
void chassis_m3508_send_motor_currents_can2(void)
{
	...
	// 配置仲裁段和数据段	
	tx_message_3.StdId = 0x200;  // 用于ID为 1 2 3 4 的电机
...
void M2006AngleIntegral(M3508_REAL_INFO *M3508_MOTOR)
{
	...
	if(M3508_MOTOR->RPM >= 0)
	{
		...
		// 滤波
		if(delta_pos > 0) 
			M3508_MOTOR->REAL_ANGLE += delta_pos*180.0f/334.0f;  // 积分	
	}
	else
	{
		if(M3508_MOTOR->ANGLE > M3508_MOTOR->LAST_ANGLE)
