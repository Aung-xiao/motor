...
void CAN1_Init(void)
{
	...
	CAN_InitStructure.CAN_Mode= 0; //模式设置：0,普通模式;1,回环模式;
	//设置波特率
	...
	CAN_InitStructure.CAN_Prescaler=2; //分频系数(Fdiv)为 brp+1
	...
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1; // 主优先级为 1
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 1; // 次优先级为 1
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
	NVIC_Init(&NVIC_InitStructure);
}
